...
function MenuModule:GetName() return L['Micromenu']; end
...
function MenuModule:OnInitialize()
    ...
    self.socialIcons = {
        BSAp = {text = BNET_CLIENT_APP},
        App = {text = BNET_CLIENT_APP},
        D3 = {text = 'Diablo 3'},
        S1 = {text = 'Starcraft Remastered'},
        S2 = {text = 'Starcraft 2'},
        WTCG = {text = 'Hearthstone'},
        Hero = {text = 'Heroes of the Storm'},
        Pro = {text = 'Overwatch'},
        WoW = {text = CINEMATIC_NAME_1},
        DST2 = {text = 'Destiny 2'},
        VIPR = {text = 'Call of Duty'},
        ODIN = {text = 'Call of Duty MW'},
        LAZR = {text = 'Call of Duty MW2'},
        W3 = {text = 'Warcraft 3 Reforged'}
    }
end
...
function MenuModule:OnEnable()
    ...
    if self.microMenuFrame == nil then
        self.microMenuFrame = CreateFrame("FRAME", L['Micromenu'],
                                          xb:GetFrame('bar'))
...
function MenuModule:Refresh()
    ...
    self.modifier = self.modifiers[xb.db.profile.modules.microMenu
                        .modifierTooltip];
    ...
    for name, frame in pairs(self.frames) do
        ...
        if name == 'menu' then
            frame:SetPoint("LEFT", xb.db.profile.modules.microMenu.iconSpacing,
                           0)
            totalWidth = totalWidth + frame:GetWidth() +
                             xb.db.profile.modules.microMenu.iconSpacing
        elseif frame:GetParent():GetName() == 'menu' then
            frame:SetPoint("LEFT", frame:GetParent(), "RIGHT",
                           xb.db.profile.modules.microMenu.mainMenuSpacing, 0)
            totalWidth = totalWidth + frame:GetWidth() +
                             xb.db.profile.modules.microMenu.mainMenuSpacing
        else
            frame:SetPoint("LEFT", frame:GetParent(), "RIGHT",
                           xb.db.profile.modules.microMenu.iconSpacing, 0)
            totalWidth = totalWidth + frame:GetWidth() +
                             xb.db.profile.modules.microMenu.iconSpacing
        end
    end
    ...
    for name, frame in pairs(self.text) do
        ...
        self.bgTexture[name]:SetColorTexture(xb.db.profile.color.barColor.r,
                                             xb.db.profile.color.barColor.g,
                                             xb.db.profile.color.barColor.b,
                                             xb.db.profile.color.barColor.a)
...
function MenuModule:CreateFrames()
    ...
    if xb.db.profile.modules.microMenu.menu then
        ...
        if self.frames.menu then self.frames.menu = nil end
    end
    ...
    if xb.db.profile.modules.microMenu.chat then
        ...
        if self.frames.chat then self.frames.chat = nil end
    end
    ...
    if xb.db.profile.modules.microMenu.char then
        ...
        if self.frames.char then self.frames.char = nil end
    end
    ...
    if xb.db.profile.modules.microMenu.spell then
        ...
        if self.frames.spell then self.frames.spell = nil end
    end
    ...
    if xb.db.profile.modules.microMenu.talent then
        ...
        if self.frames.talent then self.frames.talent = nil end
    end
    ...
    if xb.db.profile.modules.microMenu.ach then
        ...
        if self.frames.ach then self.frames.ach = nil end
    end
    ...
    if xb.db.profile.modules.microMenu.quest then
        ...
        if self.frames.quest then self.frames.quest = nil end
    end
    ...
    if xb.db.profile.modules.microMenu.lfg then
        ...
        if self.frames.lfg then self.frames.lfg = nil end
    end
    ...
    if xb.db.profile.modules.microMenu.journal then
        ...
        if self.frames.journal then self.frames.journal = nil end
    end
    ...
    if xb.db.profile.modules.microMenu.pvp then
        ...
        if self.frames.pvp then self.frames.pvp = nil end
    end
    ...
    if xb.db.profile.modules.microMenu.pet then
        ...
        if self.frames.pet then self.frames.pet = nil end
    end
    ...
    if xb.db.profile.modules.microMenu.shop then
        ...
        if self.frames.shop then self.frames.shop = nil end
    end
    ...
    if xb.db.profile.modules.microMenu.help then
        ...
        if self.frames.help then self.frames.help = nil end
    end
...
function MenuModule:IconDefaults(name)
    ...
    self.icons[name]:SetVertexColor(colors.normal.r, colors.normal.g,
                                    colors.normal.b, colors.normal.a)
end
...
function MenuModule:RegisterFrameEvents()
    ...
    self:RegisterEvent('GUILD_ROSTER_UPDATE',
                       function() self:UpdateGuildText() end)
    self:RegisterEvent('CHAT_MSG_GUILD', function() self:UpdateGuildText() end)
...
function MenuModule:DefaultLeave(name)
    return function()
        ...
        if self.icons[name] ~= nil then
            self.icons[name]:SetVertexColor(xb.db.profile.color.normal.r,
                                            xb.db.profile.color.normal.g,
                                            xb.db.profile.color.normal.b,
                                            xb.db.profile.color.normal.a)
        end
    end
end
...
function MenuModule:SocialHover(hoverFunc)
    return function()
        ...
        if totalBNOnlineFriends then
            local clientsList = {
                BNET_CLIENT_WOW, BNET_CLIENT_SC2, BNET_CLIENT_D3,
                BNET_CLIENT_WTCG, BNET_CLIENT_HEROES, BNET_CLIENT_OVERWATCH,
            ...
            for i = 1, BNGetNumFriends() do
                ...
                if isOnline then
                    ...
                    if tContains(clientsList, gameClient) or
                        not xb.db.profile.modules.microMenu.hideAppContact then
                        local lineLeft =
                            string.format("|T%s:16|t|cff82c5ff %s|r %s",
                                          statusIcon, battleName, note)
                        local lineRight =
                            string.format("%s %s |T%s:16|t", charNameFormat,
                                          gameName, socialIcon)
                        tooltip:AddLine(lineLeft, lineRight)
                        tooltip:SetLineScript(tooltip:GetLineCount(), "OnEnter",
                                              function()
                            self.lineHover = true;
                        end)
                        tooltip:SetLineScript(tooltip:GetLineCount(), "OnLeave",
                                              function()
                            self.lineHover = false;
                        end)
                        tooltip:SetLineScript(tooltip:GetLineCount(),
                                              "OnMouseUp",
                            ...
                            if button == "LeftButton" then
                                if modifierFunc() then
                                    if CanGroupWithAccount(battleID) then
                                        InviteToGroup(
                                            charName .. "-" .. realmName) -- Still working?
                                    end
                                else
                                    ChatFrame_OpenChat(
                                        "/o " .. battleName .. " ")
        ...
        if totalOnlineFriends then
            for i = 1, C_FriendList.GetNumFriends() do
                ...
                if isOnline then
                    ...
                    local lineLeft = string.format(
                                         "|T%s:16|t %s, " .. LEVEL .. ":%s %s",
                                         statusIcon, name, level, class)
        ...
        tooltip:AddLine('|cffffff00<' .. L['Left-Click'] .. '>|r',
                        '|cffffffff' .. L['Whisper BNet'] .. '|r')
        tooltip:AddLine(
            '|cffffff00<' .. self.modifier .. '+' .. L['Left-Click'] .. '>|r',
            '|cffffffff' .. CALENDAR_INVITELIST_INVITETORAID .. '|r')
        tooltip:AddLine('|cffffff00<' .. L['Right-Click'] .. '>|r',
                        '|cffffffff' .. L['Whisper Character'] .. '|r')
        if (totalOnlineFriends + totalBNOnlineFriends) > 0 then
            tooltip:Show()
        end
        hoverFunc()
...
function MenuModule:GuildHover(hoverFunc)
    return function()
        ...
        for i = 0, totalGuild do
            local name, _, _, level, _, zone, note, _, isOnline, status, class,
                  _, _, isMobile, _ = GetGuildRosterInfo(i)
            if isOnline then
                ...
                local lineLeft = string.format('%s |c%s%s|r %s %s', level,
                                               colorHex, name, status, note)
                local lineRight = string.format('%s|cffffffff %s',
                                                (isMobile and "|cffffffa0[M]|r " or
                                                    ""), zone or '')
                tooltip:AddLine(lineLeft, lineRight)
                tooltip:SetLineScript(tooltip:GetLineCount(), "OnEnter",
                                      function()
...
function MenuModule:CreateClickFunctions()
    ...
    self.functions.guild = function(self, button, down)
        ...
        if button == "LeftButton" then ToggleGuildFrame() end
    end; -- guild
    ...
    self.functions.social = function(self, button, down)
        ...
        if button == "LeftButton" then ToggleFriendsFrame() end
    end; -- social
    ...
    self.functions.spell = function(self, button, down)
        ...
        if button == "LeftButton" then ToggleFrame(SpellBookFrame) end
    end; -- spell
    ...
    self.functions.talent = function(self, button, down)
        ...
        if button == "LeftButton" then ToggleTalentFrame() end
    end; -- talent
    ...
    self.functions.journal = function(self, button, down)
        ...
        if button == "LeftButton" then ToggleEncounterJournal() end
    end; -- journal
    ...
    self.functions.lfg = function(self, button, down)
        ...
        if button == "LeftButton" then ToggleLFDParentFrame() end
    end; -- lfg
    ...
    self.functions.pet = function(self, button, down)
        ...
        if button == "LeftButton" then ToggleCollectionsJournal() end
    end; -- pet
    ...
    self.functions.ach = function(self, button, down)
        ...
        if button == "LeftButton" then ToggleAchievementFrame() end
    end; -- ach
    ...
    self.functions.quest = function(self, button, down)
        ...
        if button == "LeftButton" then ToggleQuestLog() end
    end; -- quest
    ...
    self.functions.shop = function(self, button, down)
        ...
        if button == "LeftButton" then ToggleStoreUI() end
    end; -- shop
    ...
    self.functions.help = function(self, button, down)
        ...
        if button == "LeftButton" then ToggleHelpFrame() end
    end; -- help
end
    ...
    return {
        ...
        args = {
            ...
            modifierTooltip = {
                ...
                disabled = function()
                    return not xb.db.profile.modules.microMenu.guild and
                               not xb.db.profile.modules.microMenu.social
                end
            },
            appFriendsHide = {
