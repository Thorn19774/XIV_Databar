...
function TalentModule:GetName() return TALENTS; end
...
function TalentModule:OnEnable()
    ...
    if self.talentFrame == nil then
        self.talentFrame = CreateFrame("FRAME", "talentFrame",
                                       xb:GetFrame('bar'))
...
function TalentModule:Refresh()
    ...
    self.specIcon:SetVertexColor(db.color.normal.r, db.color.normal.g,
                                 db.color.normal.b, db.color.normal.a)

    self.specText:SetFont(xb:GetFont(textHeight))
    self.specText:SetTextColor(db.color.inactive.r, db.color.inactive.g,
    ...
    if self.specBar:IsVisible() then
        ...
        self.specFrame:SetSize(iconSize + self.specText:GetWidth() + 5,
                               xb:GetHeight())
    end
    ...
    self.talentFrame:SetPoint('RIGHT', anchorFrame, relativeAnchorPoint,
                              -(xOffset), 0)
...
function TalentModule:CreateFrames()
    self.specFrame = self.specFrame or
                         CreateFrame("BUTTON", nil, self.talentFrame,
                                     'SecureActionButtonTemplate')
    self.specIcon = self.specIcon or
                        self.specFrame:CreateTexture(nil, 'OVERLAY')
    self.specText = self.specText or
                        self.specFrame:CreateFontString(nil, 'OVERLAY')
    self.specBar = self.specBar or CreateFrame('STATUSBAR', nil, self.specFrame)
    self.specBarBg = self.specBarBg or
                         self.specBar:CreateTexture(nil, 'BACKGROUND')

    self.specPopup = self.specPopup or
                         CreateFrame('BUTTON', "SpecPopup", self.specFrame)
    self.specPopup:SetFrameStrata("TOOLTIP")
    self.specPopupTexture = self.specPopupTexture or
                                self.specPopup:CreateTexture(nil, 'BACKGROUND')
    self.lootSpecPopup = self.lootSpecPopup or
                             CreateFrame('BUTTON', "LootPopup", self.specFrame)
    self.lootSpecPopup:SetFrameStrata("TOOLTIP")
    self.lootSpecPopupTexture = self.lootSpecPopupTexture or
                                    self.lootSpecPopup:CreateTexture(nil,
                                                                     'BACKGROUND')
    self:CreateSpecPopup()
    self:CreateLootSpecPopup()
end
...
function TalentModule:RegisterFrameEvents()
    ...
    self.specFrame:SetScript('OnEnter', function()
        ...
        if xb.db.profile.modules.tradeskill.showTooltip then
            if ((not self.specPopup:IsVisible()) or
                (not self.lootSpecPopup:IsVisible())) then
                self:ShowTooltip()
            end
        end
    end)
...
function TalentModule:CreateSpecPopup()
    ...
    local db = xb.db.profile
    local iconSize = db.text.fontSize + db.general.barPadding
    self.specOptionString = self.specOptionString or
                                self.specPopup:CreateFontString(nil, 'OVERLAY')
    self.specOptionString:SetFont(xb:GetFont(
                                      db.text.fontSize + self.optionTextExtra))
    self.specOptionString:SetTextColor(db.color.normal.r, db.color.normal.g,
                                       db.color.normal.b, db.color.normal.a)
    ...
    for i = 1, GetNumSpecializations() do
        if self.specButtons[i] == nil then
            ...
            buttonIcon:SetVertexColor(db.color.normal.r, db.color.normal.g,
                                      db.color.normal.b, db.color.normal.a)

            buttonText:SetFont(xb:GetFont(db.text.fontSize))
            buttonText:SetTextColor(db.color.inactive.r, db.color.inactive.g,
                                    db.color.inactive.b, db.color.inactive.a)
            ...
            button:SetScript('OnEnter', function()
                buttonText:SetTextColor(db.color.normal.r, db.color.normal.g,
                                        db.color.normal.b, db.color.normal.a)
            end)

            button:SetScript('OnLeave', function()
                buttonText:SetTextColor(db.color.inactive.r,
                                        db.color.inactive.g,
                                        db.color.inactive.b, db.color.inactive.a)
            end)
    ...
    if popupWidth < (self.specOptionString:GetStringWidth() + self.extraPadding) then
        popupWidth =
            (self.specOptionString:GetStringWidth() + self.extraPadding)
    end
    self.specPopup:SetSize(popupWidth, popupHeight + xb.constants.popupPadding)

    local popupPadding = xb.constants.popupPadding
    if db.general.barPosition == 'TOP' then popupPadding = -(popupPadding) end

    self.specPopup:ClearAllPoints()
    self.specPopupTexture:ClearAllPoints()
    self.specPopup:SetPoint(db.general.barPosition, self.specFrame,
                            xb.miniTextPosition, 0, popupPadding)
    self.specPopupTexture:SetColorTexture(db.color.barColor.r,
                                          db.color.barColor.g,
                                          db.color.barColor.b,
                                          db.color.barColor.a)
...
function TalentModule:CreateLootSpecPopup()
    ...
    local db = xb.db.profile
    local iconSize = db.text.fontSize + db.general.barPadding
    self.lootSpecOptionString = self.lootSpecOptionString or
                                    self.lootSpecPopup:CreateFontString(nil,
                                                                        'OVERLAY')
    self.lootSpecOptionString:SetFont(xb:GetFont(
                                          db.text.fontSize +
                                              self.optionTextExtra))
    self.lootSpecOptionString:SetTextColor(db.color.normal.r, db.color.normal.g,
                                           db.color.normal.b, db.color.normal.a)
    ...
    if popupWidth <
        (self.lootSpecOptionString:GetStringWidth() + self.extraPadding) then
        popupWidth = (self.lootSpecOptionString:GetStringWidth() +
                         self.extraPadding)
    end
    self.lootSpecPopup:SetSize(popupWidth,
                               popupHeight + xb.constants.popupPadding)
    ...
    self.lootSpecPopup:SetPoint(db.general.barPosition, self.specFrame,
                                xb.miniTextPosition, 0, popupPadding)
    self.lootSpecPopupTexture:SetColorTexture(db.color.barColor.r,
                                              db.color.barColor.g,
                                              db.color.barColor.b,
                                              db.color.barColor.a)
...
function TalentModule:ShowTooltip()
    ...
    tooltip:AddLine('|cFFFFFF00<' .. L['Left-Click'] .. '>|r',
                    "|cFFFFFFFF" .. L['Set Specialization'] .. "|r")
    tooltip:AddLine(
...
function TalentModule:GetDefaultOptions()
    return 'talent',
           {enabled = true, barCC = false, showTooltip = true, minWidth = 50}
end
    ...
    return {
        ...
        args = {
            ...
            barCC = {
                ...
                type = "toggle",
                get = function()
                    return xb.db.profile.modules.talent.barCC;
                end,
                set = function(_, val)
                    xb.db.profile.modules.talent.barCC = val;
                    self:Refresh();
                end
            },
            showTooltip = {
